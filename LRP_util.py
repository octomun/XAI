...
def threshold(cam):
    cam_mask = (cam > (cam.mean() + cam.std()))
    # cam_mask = (cam > (cam.mean() ))
    # cam_mask = cam_mask[..., np.newaxis]
    return cam_mask

def threshold_otsu(x):
    y = x * 255
    y = y.astype(np.uint8)
    # binary = cv2.adaptiveThreshold(y, 1, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 467, 37)

    t, t_otsu = cv2.threshold(y, -1, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    # mean_ = x.mean()
    # std_ = x.std()
    # thresh = mean_ +std_
    # x = (x>thresh)
    t_otsu = t_otsu.astype(np.float)
    return t_otsu/255
